...
/**
 * API Route: /api/admin-v2/security-metrics
 * 
 * Endpoint refactorizado para métricas de seguridad usando la nueva arquitectura.
 */
...
// GET /api/admin-v2/security-metrics - Obtener métricas de seguridad
export async function GET(request: NextRequest) {
  try {
    const controller = await getController()
    return await controller.getSecurityMetrics(request)
  } catch (error) {
    console.error('[API] Error in GET /api/admin-v2/security-metrics:', error)
